...
async def on_message(message):
    # Placeholder for Gradbot's message but don't send it yet
    msg = cl.Message(content="", author="Gradbot")
        ...
        def on_llm_end(self, response, *, run_id, parent_run_id, **kwargs):
            ...
            if self.sources:
                ...
                self.msg.elements.append(
                    cl.Text(name="Gradbot", content=content, display="inline")
                )

    # Stream the response and send chunks only when available
    final_message = ""
    async for chunk in runnable.astream(
        ...
        if chunk.strip():  # Only update if there's content
            final_message += chunk
